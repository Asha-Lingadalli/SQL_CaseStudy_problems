...
-- count(distinct case when b.line_of_business='Flight' and b.booking_date between '2022-04-01' and '2022-04-30' then b.user_id end)as book_in_flight_april22
...
-- max(booking_date) - min(booking_date) as calculate_days
-- from booking_table
-- group by user_id
-- order by user_id 
...
-- select u.segment,
-- sum(CASE WHEN b.line_of_business = 'Flight' THEN 1 else 0 end) as no_flights,
-- sum(CASE WHEN b.line_of_business = 'Hotel' THEN 1 else 0 END) as no_hotels
-- from booking_table b
